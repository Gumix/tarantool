...
static void
arrow_array_init(struct ArrowArray *array, int column_count, int row_count)
{
...
gen_rand_column_number:
	...
	arrow_array_init(array, batch_column_count, batch_row_count);

	for (int i = 0; i < batch_column_count; i++) {
		int num = column_numbers[i];
		assert(num < column_count);
		array->children[i]->buffers[1] =
			&dataset.columns[num].data[batch * batch_row_count];
	}
}
