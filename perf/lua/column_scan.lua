...
local USAGE = [[
   ...
   column_count <number, 100>       - number of columns in the test space
                                      (ignored if field_type is string)
...
local WORK_DIR = string.format(
    ...
    'str_len_min=%d,str_len_max=%d',
    params.engine, params.column_count, params.row_count, params.field_type,
...
do_or_die(box.once, 'init', function()
    ...
    s:create_index('pk')
    local gen
    if params.field_type == 'unsigned' and
       (params.engine == 'memcs' or params.engine == 'quiver') then
        gen = gen_arrow
    else
        gen = gen_insert
    end
...
local TESTS = {
    ...
    },
    {
        ...
        type = 'string',
        func = function()
            test_funcs.str(box.space.test.id, 0, 1, false, false)
        end,
    },
    {
        name = 'string_touch',
        type = 'string',
        func = function()
            test_funcs.str(box.space.test.id, 0, 1, false, true)
        end,
    },
    {
        ...
        type = 'string',
        func = function()
            test_funcs.str(box.space.test.id, 0, 1, true, false)
        end,
    },
    {
        ...
        type = 'string',
        func = function()
            test_funcs.str(box.space.test.id, 0, 1, true, true)
        end,
    },
}
